...
const isHousekeeping = (toolName, params) => HOUSEKEEPING_TOOLS.has(toolName) ||
...
export function register(api) {
    ...
    const expandWorkspace = (value) => path.resolve(value === "~" ? os.homedir() : value.startsWith("~/") ? path.join(os.homedir(), value.slice(2)) : value);
    ...
    const storeFor = (dir) => {
        ...
        if (!store) {
            store = new DevelopmentStore(resolved);
            stores.set(resolved, store);
        }
        return store;
    };
    const scoped = (ctx) => ctx.agentId === config.agentId;
    ...
    const runFor = (callId) => calls.get(callId);
    ...
    const limitReason = async (runId) => {
        if (stopping)
            return "Curiosity service has stopped.";
        if (!runId || !knownRuns.has(runId))
            return "Missing trusted run identity; autonomous tool execution is unavailable.";
        if (denied.has(runId))
            return "No developmental session is authorized for this heartbeat.";
        const run = active.get(runId);
        if (!run)
            return undefined;
        await run.billing;
        if (Date.now() >= run.deadline)
            return "Developmental session time is exhausted. Preserve a next move and stop.";
        if ((await run.store.usage24h()).tokens >= config.maxAutonomousTokensPerDay)
            return "Autonomous token budget is exhausted. Preserve a next move and stop.";
        return undefined;
    };
    api.registerTool(ctx => {
        if (!config.enabled || !scoped(ctx))
            return null;
        ...
        return createTools({ store: storeFor(workspace), config, workspace, runId: callId => runFor(callId), beforeMemoryAction: async (callId, input) => {
                if (memoryReads.has(String(input.action ?? "snapshot")))
                    return;
                ...
                if (run && !stopping && !denied.has(runId) && closeout && !run.closeoutUsed) {
                    run.closeoutUsed = true;
                    return;
                }
                throw new Error(reason);
            }, beforeAction: async (callId) => {
                const reason = await limitReason(runFor(callId));
                if (reason)
                    throw new Error(reason);
            } });
    ...
    let waking = false;
    const wake = async () => {
        if (stopping || waking || isLive() || !defaultWorkspace)
            return;
        waking = true;
        try {
            ...
            if (!request) {
                api.logger?.warn("Curiosity v2: requestHeartbeatNow is unavailable; configure a native heartbeat for the selected agent.");
                return;
            }
    ...
    api.registerService({ id, start: () => {
            ...
            timer.unref?.();
            void wake().catch(() => api.logger?.warn("Curiosity v2 initial wake check failed"));
        }, stop: () => { stopping = true; if (timer)
    ...
    api.on("before_prompt_build", async (_event, ctx) => {
        ...
        const workspace = ctx.workspaceDir ?? defaultWorkspace;
        if (!workspace)
            return { prependContext: "Curiosity v2 needs a resolved agent workspace before it can start." };
        defaultWorkspace ??= workspace;
        const store = storeFor(workspace);
        if (ctx.runId)
            knownRuns.add(ctx.runId);
        if (ctx.trigger !== "heartbeat") {
            const awareness = renderAwarenessPrompt(await store.snapshot());
            return awareness ? { prependContext: awareness } : undefined;
        }
        if (!ctx.runId)
            return { prependContext: "Curiosity v2 could not identify this run. Do not start autonomous activity; report a runtime configuration issue." };
        if (isLive() && !active.has(ctx.runId)) {
            denied.add(ctx.runId);
            return { prependContext: "Another developmental session is active. Do not start duplicate autonomous work." };
        }
        if (!active.has(ctx.runId)) {
            ...
            if (!accepted) {
                denied.add(ctx.runId);
                return { prependContext: "Curiosity v2 has no session budget left. Do not start developmental activity; HEARTBEAT_OK is appropriate." };
            }
    ...
    api.on("before_tool_call", async (event, ctx) => {
        ...
        if (runId && callId)
            calls.set(callId, runId);
        if (event.toolName === "curiosity_v2")
            return; // Its wrapper permits reads and one bounded closeout.
        if (!TOOL_NAMES.includes(event.toolName) && (!runId || (!active.has(runId) && !denied.has(runId))))
            return;
        const reason = await limitReason(runId);
        if (reason)
    ...
    api.on("llm_output", async (event, ctx) => {
        if (!config.enabled || !scoped(ctx))
            return;
        const run = active.get(event.runId) ?? finished.get(event.runId);
        if (!run)
            return;
        ...
        run.billing = run.billing.then(async () => {
            if (tokens === undefined) {
                await run.store.recordEvent({ runId: event.runId, kind: "usage_unknown", outcome: "Runtime did not report token usage" });
                return;
            }
            await run.store.addRunTokens(event.runId, tokens);
            run.usageSeen = true;
    ...
    api.on("agent_end", async (event, ctx) => {
        if (!config.enabled || !scoped(ctx) || !ctx.runId)
            return;
        denied.delete(ctx.runId);
        const run = active.get(ctx.runId);
        if (!run) {
            knownRuns.delete(ctx.runId);
            for (const [callId, runId] of calls)
                if (runId === ctx.runId)
                    calls.delete(callId);
            return;
        }
