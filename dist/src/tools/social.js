...
function visibility(value) {
    if (!VISIBILITIES.has(value))
        throw new Error("visibility must be public, unlisted, private, or direct");
    return value;
}
function text(value) {
    return required(value, "text");
}
function rejectExtraMentions(value) {
...
function id(value, name = "targetId") {
...
function status(value) {
    ...
    return {
        ...
        ...(VISIBILITIES.has(raw.visibility) ? { visibility: raw.visibility } : {}),
...
export class MastodonClient {
    ...
    async execute(input) {
        switch (input.action) {
            ...
            case "discover_public_timeline":
                return { action: input.action, statuses: (await this.request("GET", `/api/v1/timelines/public${queryParams({ limit: limit(input.limit), local: input.local, max_id: optionalString(input.maxId), min_id: optionalString(input.minId), since_id: optionalString(input.sinceId) })}`)).slice(0, MAX_ITEMS).map(status) };
            case "search": {
            ...
            case "publish":
                {
                    ...
                    if (requestedVisibility === "direct")
                        throw new Error("publish does not accept direct visibility; use the direct action with one verified recipient");
            ...
            case "direct": {
                ...
                if (input.visibility !== "direct")
                    throw new Error("direct messages require visibility=direct");
    ...
    async publish(input, allowDirect = false) {
        if (input.visibility === "direct" && !allowDirect)
            throw new Error("direct visibility requires the direct or direct-reply action");
    ...
    async request(method, path, body, idempotencyKey) {
        ...
        if (response.status === 429)
            throw new MastodonRateLimitError(response.headers.get("retry-after") ?? undefined);
        if (!response.ok) {
...
function assertNever(value) {
    throw new Error(`Unsupported Mastodon action ${value.action ?? "unknown"}`);
}
