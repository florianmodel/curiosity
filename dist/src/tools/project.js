...
export async function safePath(workspace, relative, createParents = false) {
    const root = await fs.realpath(workspace);
    const parts = relative.split(/[\\/]/);
    if (path.isAbsolute(relative) || parts.some(p => !p || p === "." || p === ".." || p.includes("\0")))
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        try {
            ...
            if (stat.isSymbolicLink())
                throw new Error("Symbolic links are not allowed in creations");
...
export async function writeManaged(workspace, relative, content, overwrite = false) {
    ...
    let file;
    try {
        file = await fs.open(target, flags, 0o600);
    }
    catch (error) {
        if (error.code === "EEXIST")
            throw new Error(`${relative} already exists; use overwrite:true deliberately`);
        throw error;
    }
    try {
...
export async function projectAction(workspace, input) {
    if (!/^[a-z0-9][a-z0-9_-]{0,80}$/.test(input.project))
        throw new Error("project must be a lowercase slug");
    ...
    if (!input.path || !EXTENSIONS.has(path.extname(input.path).toLowerCase()))
        throw new Error("Provide a supported text/code file path");
    ...
    try {
        ...
        if (!stat.isFile() || stat.size > MAX_BYTES)
            throw new Error("Not a bounded text file");
