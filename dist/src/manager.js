...
export class CuriosityManager {
    ...
    async findActiveGoalForAgent(agentId, maxAgeMs = 2 * 60 * 60 * 1000) {
        const db = await this.ensureDb();
    ...
    async canUseTool(runId, toolName, options = {}) {
    ...
    async recordObservedToolCalls(params) {
        ...
        const normalizedToolNames = uniq(params.toolNames
            .map((toolName) => toolName.trim())
            .filter(Boolean)
            .map((toolName) => toolName.replace(/^openclaw_/, "")));
        const callCount = Math.max(normalizedToolNames.length, Number.isFinite(params.callCount ?? NaN) ? Math.trunc(params.callCount ?? 0) : 0);
        ...
        for (let index = 0; index < callCount; index += 1) {
            const toolName = normalizedToolNames[index] ?? normalizedToolNames[normalizedToolNames.length - 1];
            if (!toolName) {
                continue;
            }
            const normalizedToolName = toolName.trim().toLowerCase();
    ...
    async canSendMessage(runId, surface, to, options = {}) {
        const resolvedRun = await this.resolveRunForAutonomousEvent({
            runId,
            agentId: options.agentId,
        });
    ...
    async buildCandidates(params) {
        ...
        if (this.config.boredom.enabled && params.boredom.level >= this.config.boredom.wakeLevel) {
            ...
            candidates.push({
                ...
                keywords: extractKeywords(targetSurface === "web"
        ...
        if (this.config.goalSources.externalFollowUps) {
            for (const goal of params.recentCompleted.slice(0, 4)) {
                ...
                if (goal.source === "external_follow_up" ||
    ...
    async selectGoalForRun(params) {
        ...
        const forcedSurface = params.forcedSurface?.trim().toLowerCase();
        const forcedSurfaceMatches = params.forceSelect === true
            ? ranked.filter((goal) => {
                return !forcedSurface || goal.targetSurface.trim().toLowerCase() === forcedSurface;
            })
            : [];
        const forcedSelection = forcedSurfaceMatches.find((goal) => /^Manual forced/i.test(goal.title)) ??
    ...
    async finalizeAutonomousRun(params) {
        ...
        const minimumActionError = `autonomous curiosity ended before taking ${this.config.actionPolicy.minimumSensingSteps} qualifying tool-backed step(s) or declaring no safe tool affordance`;
        const outcome = {
            success,
    ...
    async countSensingStepsForRun(runId) {
        const db = await this.ensureDb();
