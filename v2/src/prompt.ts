import type { Snapshot, V2Config } from "./types.js";
...
function compact(snapshot: Snapshot): string {
...
function affordances(config: V2Config): string {
  const lines = [
    "- `curiosity_v2`: recall your snapshot and persist state. Always available.",
  ];
  if (config.allowWebFetch) lines.push("- `curiosity_web_fetch`: read any public web page (http/https). This is how you explore beyond the workspace. Cite what you read.");
  if (config.allowNotes) lines.push("- `curiosity_note_write`: create real files under `<workspace>/creations/<month>/<slug>.md` (also txt/json/html/svg/csv/js/ts). Artifacts you write here are visible and durable.");
  if (config.allowSelfModification) lines.push("- Filesystem access via OpenClaw approvals lets you inspect and revise your own implementation, versioned and auditable.");
  return lines.join("\n");
}

function turnContract(): string {
...
function boundaries(config: V2Config): string {
...
export function renderDevelopmentPrompt(snapshot: Snapshot, config: V2Config): string {
...
export function renderAwarenessPrompt(snapshot: Snapshot): string | undefined {
  if (!snapshot.self && snapshot.interests.length === 0 && snapshot.projects.length === 0) return undefined;
