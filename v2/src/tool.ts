...
function validateTurn(record: Record<string, unknown>): string | undefined {
  if (!modes.has(String(record.mode))) return "mode must be one of wander, follow, make, participate, reflect, self_modify";
  const action = record.action as { kind?: unknown; outcome?: unknown } | undefined;
  ...
  if (!hasAction && !hasBlock) return "record.action (with kind and outcome) or record.blockedReason is required";
  if (hasAction && hasBlock) return "a turn reports either an action or a blockedReason, not both";
  return undefined;
}
  ...
  return {
    ...
    execute: async (_id: string, input: Record<string, unknown>) => {
      ...
      if (action === "record_turn") {
        ...
        if (problem) throw new Error(`Invalid turn report: ${problem}`);
      }
      ...
      if (action === "record_visit" && typeof record.kind !== "string") record.kind = "other";
      if (action === "record_turn" && typeof record.runId !== "string") delete record.runId;
