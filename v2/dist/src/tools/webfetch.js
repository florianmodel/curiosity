...
export function isBlockedUrl(raw) {
    ...
    if (host.startsWith("::ffff:")) {
        ...
        if (net.isIPv4(mapped) && blockedIPv4(mapped))
            return `host ${host} is a private address`;
    }
    return undefined;
}
...
export async function guardedFetch(raw, fetchImpl = fetch) {
    let current = raw;
    for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
        const blocked = isBlockedUrl(current);
        if (blocked)
            throw new Error(`Blocked ${current}: ${blocked}`);
...
export async function readPage(raw, fetchImpl = fetch) {
    ...
    if (!response.ok && text.length === 0)
        throw new Error(`HTTP ${response.status} with empty body at ${finalUrl}`);
    return {
        url: raw,
        finalUrl,
