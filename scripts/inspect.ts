...
if(args.includes("--help")) {
  console.log("Usage: node dist/scripts/inspect.js --workspace /agent/workspace [--limit 100] [--before event-sequence] [--id record-id] [--html /output/timeline.html]");process.exit(0);
}
...
const records=db.prepare("SELECT kind,id,updated_at,body_json FROM records ORDER BY updated_at DESC LIMIT ?").all(count) as Array<{kind:string;id:string;updated_at:number;body_json:string}>;
const events=tables.has("events")?db.prepare("SELECT seq,body_json FROM events WHERE seq<? ORDER BY seq DESC LIMIT ?").all(before,count) as Array<{seq:number;body_json:string}>:[];
...
if(!htmlPath){console.log(JSON.stringify(data,null,2));process.exit(0);}
const escape=(value:unknown)=>String(value??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[c]!);
const details=(value:unknown)=>`<details><summary>Details</summary><pre>${escape(JSON.stringify(value,null,2))}</pre></details>`;
...
const output=path.resolve(htmlPath);await fs.writeFile(output,html,"utf8");console.log(`Wrote ${output}`);
