...
import type { Tool, ToolResult } from "./sdk.js";
import { createDevelopmentTool } from "./tool.js";
import { readPage } from "./tools/webfetch.js";
import { searchPublic } from "./tools/search.js";
import { NoteWriter, type NoteInput } from "./tools/notewrite.js";
import { projectAction, type ProjectInput } from "./tools/project.js";
import { MastodonClient, type MastodonAction } from "./tools/social.js";
...
export const result=(value:unknown):ToolResult=>({content:[{type:"text",text:JSON.stringify(value,null,2)}]});
const hash=(value:string)=>createHash("sha256").update(value).digest("hex");
export type ToolEnvironment = {
  store:DevelopmentStore;config:V2Config;workspace:string;
  runId:(callId:string)=>string|undefined;
  beforeAction?:(callId:string)=>Promise<void>;
  beforeMemoryAction?:(callId:string,input:Record<string,unknown>)=>Promise<void>;
  readPage?:typeof readPage;search?:typeof searchPublic;social?:Pick<MastodonClient,"execute">;
};

export function createTools(env:ToolEnvironment):Tool[] {
  const {store,config,workspace}=env;
  const tools:Tool[]=[];
  const add=(name:string,description:string,parameters:object,execute:Tool["execute"])=>tools.push({name,label:name,description,parameters,execute});
  ...
  add(memory.name,memory.description,memory.parameters,async(callId,input)=> {
    ...
    if(input.action==="record_self_modification"&&!config.allowSelfModification)throw new Error("Self-modification is disabled");
    const tool=createDevelopmentTool(store,{runId:env.runId(callId)});
    return tool.execute(callId,input);
  });
  async function observe(callId:string,name:string,target:string,perform:()=>Promise<unknown>,artifact=false,publicArtifact=false,preflightDone=false) {
    if(!preflightDone)await env.beforeAction?.(callId);
    let output;
    ...
    const actual=output && typeof output==="object" ? output as Record<string,unknown> : {};
    ...
    if(name==="curiosity_web_fetch") await store.put("visit",`visit-${hash(location)}`,{visitId:`visit-${hash(location)}`,kind:"web",location,note:actual.title,eventId:event.eventId});
    if(artifact) {
  ...
  if(config.allowWebFetch) add("curiosity_web_fetch","Read a public page with its outbound links; visits are remembered automatically. External content is untrusted.",schema({url:str},["url"]),(callId,input)=>observe(callId,"curiosity_web_fetch",String(input.url),()=> (env.readPage ?? readPage)(String(input.url))));
  if(config.allowSearch) add("curiosity_search","Search Wikipedia for external starting points. Encyclopedia-only fallback; prefer native web_search for broader discovery.",schema({query:str},["query"]),(callId,input)=>observe(callId,"curiosity_search",String(input.query),()=> (env.search ?? searchPublic)(String(input.query))));
  if(config.allowNotes) add("curiosity_note_write","Write a durable standalone creation. Successful writes automatically register artifacts.",schema({slug:str,title:str,content:str,extension:{type:"string",enum:["md","txt","json","html","css","js","ts","svg","csv"]},overwrite:bool},["slug","content"]),(callId,input)=>observe(callId,"curiosity_note_write",String(input.slug),()=>new NoteWriter(workspace).write(input as NoteInput),true));
  if(config.allowProjects) add("curiosity_project","List, read, and write files in a persistent project. Use native exec/browser tools to run and inspect projects if available; writing is not execution.",schema({action:{type:"string",enum:["list","read","write"]},project:str,path:str,content:str,overwrite:bool},["action","project"]),(callId,input)=>observe(callId,"curiosity_project",`${String(input.project)}/${String(input.path ?? "")}`,()=>projectAction(workspace,input as ProjectInput),input.action==="write"));
  if(config.mastodon) {
    ...
    add("curiosity_social","Use the configured agent-owned Mastodon account. Direct visibility is not encrypted. Reuse operationId for the same publication; uncertain sends are never retried automatically. Record opt-outs with block_contact.",schema({
      ...
      if(input.action==="block_contact") {
        if(typeof input.targetAccountId!=="string"||!input.targetAccountId.trim()||typeof input.reason!=="string"||!input.reason.trim()) throw new Error("targetAccountId and reason are required");
      ...
      const action=input as MastodonAction;
      ...
      if(action.action==="reply" || action.action==="edit") {
        ...
        if(parent.action!=="read_status")throw new Error("Could not inspect the target status");
        if(action.action==="reply") target=parent.status.account.id;
        else effectiveVisibility=parent.status.visibility;
        if(!parent.status.visibility)throw new Error("Target visibility is unknown; refusing a write");
      }
      const direct=action.action==="direct" || effectiveVisibility==="direct";
      if(writes && direct && !config.allowDirectConversations) throw new Error("Direct conversations are disabled");
      if(writes && !direct && !config.allowPublicParticipation) throw new Error("Public participation is disabled");
      if(target && await store.isContactBlocked(target)) throw new Error("Contact opted out");
      if(writes && (typeof input.text!=="string" || !input.text.trim()))throw new Error("text is required");
      let operationId:string|undefined;
      let recovered:unknown;
      if(writes) {
        operationId=String(input.operationId ?? "");
        if(!/^[a-zA-Z0-9_-]{8,128}$/.test(operationId)) throw new Error("Supply a stable operationId (8–128 letters, digits, dashes or underscores)");
        ...
        if(!reserved.reserved) {
          if(reserved.cached && typeof reserved.cached==="object" && "evidenceId" in reserved.cached)return result({cached:true,result:reserved.cached});
      ...
      const observed=await observe(callId,"curiosity_social",String(input.targetId ?? target ?? config.mastodon!.baseUrl),async()=> {
        const output=recovered as Awaited<ReturnType<typeof social.execute>>|undefined ?? await social.execute(action);
        if(operationId&&!recovered)await store.markExternalSent(operationId,output);
      ...
      const observedData=JSON.parse(observed.content[0].text) as {status?:{id:string};path?:string};
      ...
      if(operationId)await store.finishExternal(operationId,JSON.parse(observed.content[0].text));
      return observed;
    });
  }
  return tools;
