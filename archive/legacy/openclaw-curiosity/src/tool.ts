...
function renderText(title: string, payload: unknown): string {
...
export function createCuriosityInspectTool(params: {
  ...
  return {
    ...
    description:
      "Inspect the curiosity goal queue, compare scorer behavior, or pause/resume autonomous curiosity.",
    ...
    execute: async (_toolCallId: string, rawParams: Record<string, unknown>) => {
      ...
      if (action === "compare") {
        const window =
          typeof rawParams.window === "string" ? parseWindowDuration(rawParams.window) : parseWindowDuration(undefined);
