...
function prettifySurface(surface) {
    return surface
        .trim()
        .replace(/[_\-]+/g, " ")
        .replace(/\s+/g, " ")
...
function buildSelfAuthoredTitle(surface, reason) {
    const readableSurface = titleCase(prettifySurface(surface));
    return `${reason}: ${readableSurface}`;
}
        ...
        for (const surface of this.configuredSurfaces()) {
            ...
            if (seenCount <= 1 || !recentChannels.has(surface)) {
                candidates.push({
                    ...
                    title: buildSelfAuthoredTitle(surface, "Self-author on underexplored surface"),
                    evidence: [
        ...
        return [
            {
                ...
                title: "Bootstrap curiosity from an empty workspace",
                evidence: [
    ...
    async buildCandidates(params) {
        ...
        if (this.config.boredom.enabled && params.boredom.level >= this.config.boredom.wakeLevel) {
            ...
            candidates.push({
                ...
                evidence: [
                    `Idle for ${idleMinutes} minute(s) without a meaningful external signal.`,
        ...
        if (this.config.goalSources.newlyDiscoveredEntities) {
            ...
            for (const [keyword, frequency] of [...keywordFrequency.entries()].filter(([, count]) => count === 1).slice(0, 5)) {
                candidates.push({
                    source: "new_entity",
                    title: `Newly surfaced topic: ${keyword}`,
    ...
    async finalizeAutonomousRun(params) {
        ...
        const outcome = {
            ...
            error: metMinimumAction
                ? params.error
                : "autonomous curiosity ended before taking a tool-backed action or declaring no tool affordance",
    ...
    autonomousRunMetMinimumAction(db, runId) {
        const eventRows = db
         ...
         WHERE run_id = ? AND event_type IN ('tool_allowed', 'external_action')`)
            .all(runId);
        const tookRealAction = eventRows.some((row) => {
